<?php
     ...
     session_start();
     //session_destroy();
     // $_SESSION['T']['inferieure']= array();
     // $_SESSION['T']['superieur']= array();
                      ...
                      <?php if($nbrPage>3){echo '<button style="position: relative; top: 20px;padding:10px;background-color:green;" type="submit" name="pagPrec1">Precedent</button>';} ?>
                       <?php if($paginf!=""){
                          for ($i=($nbrPage-3)+1; $i <= $nbrPage; $i++) { 
                             echo '<a class="pagPag" href="index2.php?nume=0&page='.$i.'">'.($i).'</a>';
                          }
                      }?> 

                      <?php if (isset($_SESSION['T']['inferieure'])) {
                        if($nbrPage!=ceil( counte($_SESSION['T']['inferieure'])/100 ) && $paginf!=""){echo '<button style="position: relative; top: 20px;left: 10px;padding:10px;background-color:green;" type="submit" name="pagSuiv1">Suivant</button>';} 
                      } ?>
                      ...
                      <?php if($nbrPagesup>3){echo '<button style="position: relative; top: 20px;padding:10px;background-color:green; " type="submit" name="pagPrec2">Precedent</button>';} ?>                                   
                       <?php if($pagsup!=""){
                          for ($i=($nbrPagesup-3)+1; $i <= $nbrPagesup; $i++) { 
                             echo '<a class="pagPag" href="index2.php?nume=0&pagePagiSup='.$i.'">'.($i).'</a>';
                          }
                      }?>  
                     <?php if (isset($_SESSION['T']['superieur'])) {
                       if($nbrPagesup!=ceil( counte($_SESSION['T']['superieur'])/100 ) && $paginf!=""){echo '<button style="position: relative;background-color:green; top: 20px;left: 10px;padding:10px;" type="submit" name="pagSuiv2">Suivant</button>';}
                     } ?> 
            </form>
        </div>
        </div>
